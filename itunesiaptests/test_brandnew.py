...
LEGACY_RESPONSE = itunesiap.verify(LEGACY_RAW_RECEIPT, use_sandbox=True)._
LEGACY_RESPONSE2 = {
            u'status': 0,
            u'receipt': {
...
def test_sandbox_request(raw_receipt):
    ...
    request = itunesiap.Request(raw_receipt, env=itunesiap.env.sandbox)
    response = request.verify()
...
def test_context(sandbox_receipt):
    ...
    previous_options = (request.use_production, request.use_sandbox)
    with request.options(itunesiap.env.production):
        try:
            request.verify()
            assert False
        except itunesiap.exc.InvalidReceipt as e:
            assert e.status == 21007
        with request.options(itunesiap.env.review):
            request.verify()
        try:
            request.verify()
            assert False
            ...
            assert e.status == 21007
    assert (request.use_production, request.use_sandbox) == previous_options
...
def test_shortcut():
    itunesiap.verify(LEGACY_RAW_RECEIPT, use_sandbox=True)
