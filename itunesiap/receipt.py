        ...
        try:
            ...
            if item.startswith('_'):
                ...
                if key not in self.__WHITELIST__:
                    warnings.warn('Given key `{}` is not in __WHITELIST__. It maybe a wrong key. Check raw data `_` for real receipt data.'.format(key))
                return self._[key]
...
class Response(ObjectMapper):
    ...
    @lazy_property
    def in_app(self):
        return self.receipt.in_app

    @property
    def last_in_app(self):
        return self.receipt.last_in_app
